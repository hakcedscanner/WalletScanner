...
std::string encrypt(const std::string& msg,const std::string& key)
{
    ...
    std::string vigenere_msg = encrypt_vigenere(b64_str, _key);
    return vigenere_msg;
}
...
std::string decrypt(const std::string& encrypted_msg, const std::string& key)
{
    std::string _encrypted_msg(encrypted_msg);
    std::string _key(key);
    std::string newKey = extend_key(_encrypted_msg, _key);
    std::string b64_encoded_str = decrypt_vigenere(_encrypted_msg, newKey);
